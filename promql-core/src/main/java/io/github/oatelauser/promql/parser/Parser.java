...
import io.github.oatelauser.promql.functions.Function;
...
import io.github.oatelauser.promql.lexer.Item;
import io.github.oatelauser.promql.lexer.ItemType;
import io.github.oatelauser.promql.lexer.Lexer;
import io.github.oatelauser.promql.posrange.PositionRange;
import io.github.oatelauser.promql.util.DurationFormat;
...
public final class Parser {
    ...
    private Expr checkAST(Expr node) {
        if (node instanceof AggregateExpr a) {
            Expr expr2 = expectType(a.expr(), io.github.oatelauser.promql.value.ValueType.VECTOR, "aggregation expression");
            Expr param2 = a.param();
            AggregateOp op = a.op();
            if (op == AggregateOp.TOPK || op == AggregateOp.BOTTOMK || op == AggregateOp.QUANTILE
                    || op == AggregateOp.LIMITK || op == AggregateOp.LIMIT_RATIO) {
                param2 = expectType(a.param(), io.github.oatelauser.promql.value.ValueType.SCALAR, "aggregation parameter");
            }
            if (op == AggregateOp.COUNT_VALUES) {
                param2 = expectType(a.param(), io.github.oatelauser.promql.value.ValueType.STRING, "aggregation parameter");
            }
            ...
            if (e2.type() != io.github.oatelauser.promql.value.ValueType.SCALAR && e2.type() != io.github.oatelauser.promql.value.ValueType.VECTOR) {
            ...
            if (e2.type() != io.github.oatelauser.promql.value.ValueType.VECTOR) {
                addParseErrf(node.positionRange(), String.format(
    ...
    private Expr expectType(Expr node, io.github.oatelauser.promql.value.ValueType want, String context) {
    ...
    private Expr checkBinary(BinaryExpr b) {
        ...
        io.github.oatelauser.promql.value.ValueType lt = lhs2.type();
        io.github.oatelauser.promql.value.ValueType rt = rhs2.type();
        ...
        if (b.op().isComparisonOperator() && !b.returnBool()
                && lt == io.github.oatelauser.promql.value.ValueType.SCALAR
                && rt == io.github.oatelauser.promql.value.ValueType.SCALAR) {
        ...
        if (lt != io.github.oatelauser.promql.value.ValueType.SCALAR && lt != io.github.oatelauser.promql.value.ValueType.VECTOR) {
            addParseErrf(b.lhs().positionRange(),
                    "binary expression must contain only scalar and instant vector types");
        }
        if (rt != io.github.oatelauser.promql.value.ValueType.SCALAR && rt != io.github.oatelauser.promql.value.ValueType.VECTOR) {
            addParseErrf(b.rhs().positionRange(),
                    "binary expression must contain only scalar and instant vector types");
        }

        if ((lt != io.github.oatelauser.promql.value.ValueType.VECTOR || rt != io.github.oatelauser.promql.value.ValueType.VECTOR) && vm != null) {
            if (!matching.isEmpty()) {
        ...
        if ((lt == io.github.oatelauser.promql.value.ValueType.SCALAR || rt == io.github.oatelauser.promql.value.ValueType.SCALAR)
                && b.op().isSetOperator()) {
    ...
    private Expr checkCall(Call c) {
        ...
        if ("info".equals(fn.name()) && c.args().size() > 1) {
            ...
            if (arg1.type() != io.github.oatelauser.promql.value.ValueType.VECTOR) {
                addParseErrf(c.positionRange(), String.format("expected type %s in %s, got %s",
                        io.github.oatelauser.promql.value.ValueType.VECTOR.documentedType(),
